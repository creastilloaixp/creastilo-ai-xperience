...
Creastilo AI Xperience is a futuristic AI solutions platform built with React,
TypeScript, and Vite. The app showcases multiple AI-powered tools including a
gamified lead generation roulette, CRM dashboard, Gen-AI chat assistant, and
creative AI studio powered by Google's Gemini API.
...
**Important**: The Vite config exposes the Gemini API key as
`process.env.API_KEY` and `process.env.GEMINI_API_KEY` at runtime via define
configuration.
...
- `App.tsx` is the main orchestrator with routing logic and modal state
  management
...
**`services/geminiService.ts`**: Centralized service managing all Gemini API
interactions
...
- Model selection: `gemini-2.5-flash` for speed, `gemini-3-pro-preview` for
  complex reasoning
...
- Function calling pattern for external integrations (currently n8n email
  webhook)
...
- `ToolType`: Enum for tool navigation (Roulette, Avatar, CRM, Custom, GenAI,
  Neural)
...
2. **Live Audio Chat**: Real-time bidirectional voice conversation
   ...
   - Implemented in `Jarvis.tsx` with ScriptProcessorNode (legacy API, consider
...
- For image/voice features that may use different API keys, re-instantiate
  client
...
- Vercel configuration (`vercel.json`) rewrites all routes to `/index.html` for
  SPA routing
...
- The onboarding localStorage check is commented out for demo purposes
  (App.tsx:21-27)
- API keys are exposed client-side via Vite define - consider backend proxy for
  production
...
- Device-ID validation mentioned in code but implementation not found in current
  files
...
- **API Keys Exposed Client-Side**: [FIXED] API keys are now removed from
  `vite.config.ts` and calls are proxied through Vercel Edge Functions
  (`/api/gemini/*`).
  - **Note**: Use `npx vercel dev` for local development to support backend
    functions.

- **Supabase Keys Exposed**: `VITE_SUPABASE_ANON_KEY` is public (expected for
  frontend) but ensure Row Level Security (RLS) is enabled
  - **Location**: `.env:4`

**Medium:**

- **N8N Webhook URL Hardcoded**: Webhook URL in `geminiService.ts:6` is
  hardcoded
...
- **No Centralized State**: Multiple components using localStorage + custom
...
- **localStorage as Database**: Lead/prize data stored in localStorage
  (client-side only)
...
- **Loose Typing**: Multiple `any` types, especially in Gemini service
  ...
  - **Solution**: Define proper interfaces for Gemini responses, lead data
    structures
...
- **Legacy API Usage**: `ScriptProcessorNode` (deprecated) in `Jarvis.tsx:161`
  - **Solution**: Migrate to AudioWorklet for better performance and less
    main-thread blocking

**Bundle Size:**

- **No Code Splitting**: Single bundle loads all tools upfront
  - **Solution**: Lazy load tool components:
    `const GenAIStudio = lazy(() => import('./Tools/GenAIStudio'))`
...
- **Critical Flows to Test**: Roulette probability calculations, CRM data sync,
  Gemini API failures

**Form Validation:**

- **Weak Validation**: `Roulette.tsx:65` only checks length, no format
  validation
...
- **Fixed Heights**: `CRMTool.tsx:696` hardcoded 800px height breaks on small
  screens
...
- Device-ID validation mentioned (`Roulette.tsx:45`, `ToolsSection.tsx:14`) but
  not implemented
- **Solution**: Implement browser fingerprinting (FingerprintJS) or server-side
  IP tracking
